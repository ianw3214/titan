...
Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList2> CommandQueue::GetCommandList()
{
    ...
    // Either use a command allocator that is no longer "in-flight"
    //  or create a new allocator if no existing allocator can be used
    ...
    // Associate the command allocator w/ the command list so it can be retrieved when
...
uint64_t CommandQueue::ExecuteCommandList(Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList2> commandList)
{
    ...
    // Ownership of command allocator has been transferred to ComPtr in allocator queue
    //  - it is safe to release the reference in the temporary COM pointer here
